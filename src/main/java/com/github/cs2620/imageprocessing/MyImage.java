...
public class MyImage {
  ...
  void ditherBW() {
    ...
    for (int y = 0; y < newImage.getHeight(); y++) {
      ...
      for (int x = 0; x < newImage.getWidth(); x++) {
        ...
        //error now holds the error I need to spread
  ...
  void basicGreenScreen() {
    BufferedImage newImage = new BufferedImage(bufferedImage.getWidth(), bufferedImage.getHeight(), BufferedImage.TYPE_4BYTE_ABGR);
    BufferedImage mask = basicGreenScreenMask();
    ...
    for (int y = 0; y < mask.getHeight(); y++) {
      for (int x = 0; x < mask.getWidth(); x++) {
        ...
        if (maskP.isWhite()) {
          ...
          newImage.setRGB(x, y, new Pixel(255, 0, 255).getRGB());
        }
  ...
  void setBasicGreenScreenMask() {
    bufferedImage = basicGreenScreenMask();
  }

  void setAdvancedGreenScreenMask() {
    bufferedImage = advancedGreenScreenMask();
  }

  private BufferedImage basicGreenScreenMask() {
    ...
    for (int y = 0; y < mask.getHeight(); y++) {
      for (int x = 0; x < mask.getWidth(); x++) {
        ...
        int color = bufferedImage.getRGB(x, y);
        Pixel p = new Pixel(color);
        if (p.isChroma(0, 255, 0, 0)) {
          mask.setRGB(x, y, new Pixel(0, 0, 0).getRGB());
        } else {
          mask.setRGB(x, y, new Pixel(255, 255, 255).getRGB());
        }
      }
    }
    return mask;
  ...
  private BufferedImage advancedGreenScreenMask() {
    BufferedImage mask = new BufferedImage(bufferedImage.getWidth(), bufferedImage.getHeight(), BufferedImage.TYPE_4BYTE_ABGR);
  ...
  void YUVGreenScreen() {
    BufferedImage newImage = new BufferedImage(bufferedImage.getWidth(), bufferedImage.getHeight(), BufferedImage.TYPE_4BYTE_ABGR);
    BufferedImage mask = YUVGreenScreenMask();
